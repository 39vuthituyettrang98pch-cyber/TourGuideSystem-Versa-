...
public sealed class QrController : Controller
{
    ...
    {
        var scanLanguageCode = NormalizeLanguageCode(lang ?? Request.Cookies["versa.dukhach.lang"]);

        if (string.IsNullOrWhiteSpace(value))
        {
            TempData["DuKhachErrorMessage"] = "Không đọc được nội dung mã QR.";
            return RedirectToAction(nameof(Scan), new { lang = scanLanguageCode });
        }
        ...
        var languageCode = ResolveOpenLanguageCode(lang, Request.Cookies["versa.dukhach.lang"]);
    ...
    public async Task<IActionResult> PoiImage(int id, string? lang = null, bool download = false, CancellationToken cancellationToken = default)
    {
        ...
        var languageCode = ResolveQrPayloadLanguageCode(lang);
    ...
    {
        if (IsAutoLanguageRequest(languageCode))
            return "vi";
    ...
    private static string ResolveOpenLanguageCode(string? requestedLanguageCode, string? cookieLanguageCode)
    {
        return IsAutoLanguageRequest(requestedLanguageCode)
            ? "auto"
            : NormalizeLanguageCode(requestedLanguageCode ?? cookieLanguageCode);
    }

    private static string ResolveQrPayloadLanguageCode(string? languageCode)
    {
        return string.IsNullOrWhiteSpace(languageCode) || IsAutoLanguageRequest(languageCode)
            ? "auto"
