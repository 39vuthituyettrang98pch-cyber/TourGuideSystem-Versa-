...
public sealed class ApprovalController : Controller
{
    ...
    {
        var logQuery = _context.AdminActivityLogs
            .AsNoTracking()
            .Include(log => log.User)
            .Where(log =>
                log.TargetTable == "pois" &&
                log.TargetId != null &&
                (log.Action.Contains("ApprovePoi") || log.Action.Contains("RejectPoi")));

        if (status == "Approved")
        {
            logQuery = logQuery.Where(log => log.Action.Contains("ApprovePoi"));
        }
        else if (status == "Rejected")
        {
            logQuery = logQuery.Where(log => log.Action.Contains("RejectPoi"));
        }
        ...
        var poiNames = await _context.Pois
            ...
            {
                poi.Id,
                Name = poi.Translations
                    .Where(translation => translation.LanguageCode == "vi")
                    .Select(translation => translation.Name)
                    .FirstOrDefault() ?? poi.Translations.Select(translation => translation.Name).FirstOrDefault() ?? ("POI #" + poi.Id),
                poi.AdminNote
            })
            .ToDictionaryAsync(item => item.Id, item => new { item.Name, item.AdminNote }, cancellationToken);

        var items = logs.Select(log =>
        {
            var poiId = log.TargetId ?? 0;
            var isRejected = log.Action.Contains("RejectPoi", StringComparison.OrdinalIgnoreCase);
            poiNames.TryGetValue(poiId, out var poiInfo);
            var reason = isRejected
                ? ExtractRejectReason(log.Description) ?? poiInfo?.AdminNote
                : log.Description;

            return new ReviewerAuditHistoryItemViewModel
            {
                LogId = log.Id,
                PoiId = poiId,
                PoiName = poiInfo?.Name ?? $"POI #{poiId}",
                Decision = isRejected ? "Rejected" : "Approved",
                Reason = reason,
                ReviewerName = log.User?.Username,
        ...
        if (!string.IsNullOrWhiteSpace(q))
        {
            ...
            items = items.Where(item =>
                ...
                item.PoiId.ToString() == keyword.TrimStart('#') ||
                (item.ReviewerName?.Contains(keyword, StringComparison.OrdinalIgnoreCase) ?? false) ||
                (item.Reason?.Contains(keyword, StringComparison.OrdinalIgnoreCase) ?? false))
                .ToList();
        }

        var model = new ReviewerAuditHistoryViewModel
        {
            StatusFilter = status,
            Search = q,
            TotalCount = items.Count,
            ApprovedCount = items.Count(item => item.Decision == "Approved"),
            RejectedCount = items.Count(item => item.Decision == "Rejected"),
            Items = items
        };
    ...
    private static string? ExtractRejectReason(string? description)
    {
        if (string.IsNullOrWhiteSpace(description))
            return null;

        var marker = description.IndexOf(':');
        return marker >= 0 && marker + 1 < description.Length
