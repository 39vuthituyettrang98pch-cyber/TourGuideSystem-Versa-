...
public sealed class LeaderboardApiController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = stats.Select((item, index) => new LeaderboardItemDto
        {
            Rank = index + 1,
            DisplayName = string.IsNullOrWhiteSpace(names.GetValueOrDefault(item.TouristId))
                ? $"Du khách #{item.TouristId}"
                : names[item.TouristId],
